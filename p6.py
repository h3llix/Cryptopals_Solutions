...
with open("6.txt") as f:
    with open("out.txt", "w") as f1:
        for line in f:
            f1.write(base64.b64decode(line))


def hamming_distance(str1, str2):
    bytes_1 = [byte for byte in bytearray(str1)]
    bytes_2 = [byte for byte in bytearray(str2)]
    xorred_bytes = [b1 ^ b2 for b1, b2 in zip(bytes_1, bytes_2)]
    hammingdistance = 0
    for byte in xorred_bytes:
        for bit in bin(byte):
            if bit == "1":
                hammingdistance += 1
